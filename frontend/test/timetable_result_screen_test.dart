...
void main() {
  ...
  testWidgets(
    'candidate tabs render the candidate matching the selected index',
    (tester) async {
      useLargeViewport(tester);
      final flow = AppFlowState()
        ..rankedCandidates = {
      ...
      expect(find.byKey(const ValueKey('candidate-2')), findsNothing);
      expect(find.text('3순위'), findsNothing);

      await tester.tap(find.text('2순위'));
      await tester.pump();

      expect(find.byKey(const ValueKey('candidate-1')), findsNothing);
      expect(find.byKey(const ValueKey('candidate-2')), findsOneWidget);
    },
  );

  testWidgets(
    'final screen renders selected ranked candidate details after server selection',
    (tester) async {
      useLargeViewport(tester);
      final flow = AppFlowState()
        ..sessionId = 'session-1'
      ...
      final api = PlanuApi(
        ...
        client: MockClient((request) async {
          ...
          expect(
            request.url.path,
            '/sessions/session-1/timetables/candidate-1/select',
          );
      ...
      await tester.scrollUntilVisible(
        find.byKey(const Key('selectTimetableButton')),
        300,
